export class Constants {

  //  public static API_ENDPOINT='http://127.0.0.1:7777/api/';

  public static API_URL: string = 'http://192.168.1.5:8080/joyBeesMobileService/';
  // public static API_URL: string = 'http://localhost:8080/joyBeesMobileService/';
  // public static API_URL: string = 'http://107.155.152.113:8190/joyBeesMobileService/';

  public static ID_USUARIO = 'id_usuario';
  public static USER_ID_FACEBOOK = 'id_usuario_facebook';
  public static DADOS_USUARIO_FACEBOOK = 'dados_usuario_Facebook';
  public static TOKEN_USUARIO = 'token_usuario';
  public static TOKEN_PUSH = 'token_push';
  public static IDIOMA_USUARIO = 'idioma_usuario';
  public static TIPO_USUARIO = 'tipo_usuario';
  public static NOME_PESSOA = 'nome_pessoa';
  public static CAMERA_DATA = 'camera_data';
  public static CAMERA_URL = 'camera_url';
  public static ID_VAGA_CANDIDATAR = 'id_vaga_candidatar';
  public static APP_NAME = 'app_name';
  public static VERSION_CODE = 'version_code';
  public static VERSION_NUMBER = 'version_number';
  public static IS_CADASTRO_COMPLETO_VAGA = 'is_cadastro_completo_vaga';
  public static IS_CADASTRO_COMPLETO_SERVICO = 'is_cadastro_completo_servico';
}