...
export class DetalheVagaPage {
  ...
  presentToast() {
    let toast = this.toastCtrl.create({
      message: this.languageDictionary.MESSAGE_PARABENS_CANDIDATADO,
      duration: 3000,
      position: 'bottom',
  ...
  candidatarVaga(idVaga) {
    try {
      ...
      // this._storage.get(Constants.IS_CADASTRO_COMPLETO).then((isCadastroCompleto) => {
        if(!localStorage.getItem(Constants.IS_CADASTRO_COMPLETO)){
          this.showAlertCadastroCompleto();
        }
        else if(localStorage.getItem(Constants.IS_CADASTRO_COMPLETO)) {
          this.loading = this.loadingCtrl.create({
          ...
          this.vagaService.callCandidatarVaga(idVaga)
            .then((vagaDetalheEntityResult: VagaDetalheEntity) => {
              ...
              this.messagePresentToast = this.languageDictionary.MESSAGE_VAGA_DESCARTADA;
  ...
  showAlertCadastroCompleto() {
    let alert = this.alertCtrl.create({
      ...
      subTitle: this.languageDictionary.MESSAGE_SUBTIBLE_CADASTRO_INCOMPLETO,
      buttons: ['OK']
    });
    alert.present();
  }
