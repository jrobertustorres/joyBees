...
export class NovoOrcamentoPage {
  ...
  private listServicoResposta1: any[] = [];
      ...
      this.servicoService.findMapServico(this.servicoListEntity)
        .then((servicoListEntityResult: ServicoListEntity) => {
          ...
          for (let i=0; i < this.listaServicos.length; i++) {
            this.listServicoResposta.push(this.listaServicos[i].listServicos);
          }
          for (let j=0; j < this.listServicoResposta.length; j++) {
            console.log(this.listServicoResposta[j]);
            // this.listServicoResposta = this.listServicoResposta[j];
            this.listServicoResposta1.push(this.listServicoResposta[j]);
          }
          for (let k=0; k < this.listServicoResposta1.length; k++) {
            console.log(this.listServicoResposta1[k].tipoServico);
            // this.listServicoResposta1 = this.listServicoResposta1[k];
  ...
  servicosChecked(idServico) {
    console.log(this.servicosSelecionados.indexOf(idServico));
