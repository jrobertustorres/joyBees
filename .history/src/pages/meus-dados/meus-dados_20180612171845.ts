...
export class MeusDadosPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dadosUsuarioForm = this.formBuilder.group({
      ...
      'salario': ['', [Validators.required, Validators.maxLength(100)]],
    ...
    // this._storage.get(Constants.ID_USUARIO).then((isLoggedIn) => {
      console.log('Is Logged in : ', localStorage.getItem(Constants.TOKEN_USUARIO));
      // this.loggedIn = isLoggedIn;
      // if(!this.loggedIn){
      if(!localStorage.getItem(Constants.TOKEN_USUARIO)){
        console.log('222222222222222222222222222 ');// VER ISSO AQUI DEPOIS
        this.dadosUsuarioForm.get('senhaUsuario').setValidators([Validators.required]);
      }
