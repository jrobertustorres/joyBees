    ...
    try {
      ...
      this.usuarioService
        ...
        .then((dadosUsuarioDetalheResult) => {
          ...
          // this.usuarioDetalheEntity.salario = salario.replace(/(\d{1,2})$/, '.$1');
          // this.usuarioDetalheEntity.salario = salario.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');



          // this.salario = this.usuarioDetalheEntity.salario.toString().split( /(?=(?:\d{3})+(?:\.|$))/g ).join( "," );
