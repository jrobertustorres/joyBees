    ...
    try {
      ...
      this.usuarioService
        ...
        .then((dadosUsuarioDetalheResult) => {
          ...
          let salario = JSON.stringify(this.usuarioDetalheEntity.salario);
          salario = salario.replace(/(\d{1,2})$/, '.$1');; 
          salario = salario.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'); 
          // this.usuarioDetalheEntity.salario = parseInt(salario);
          console.log(salario);
          // this.usuarioDetalheEntity.salario = this.maskMoney.maskConvert(this.usuarioDetalheEntity.salario);

          console.log(this.usuarioDetalheEntity.salario);
