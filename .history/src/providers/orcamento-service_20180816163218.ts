...
export class OrcamentoService {
  ...
  public findServicoOrcamentoByStatusFilter(orcamentoEntity) {
    try {

      return this._http.post(Constants.API_URL + 'findServicoOrcamentoByStatusFilter/'
        + localStorage.getItem(Constants.TOKEN_USUARIO),
        JSON.stringify(orcamentoEntity), this.options)
        .map(res => res.json())
        .toPromise()
  ...
  public lancarOrcamentoServicoPublicidade(orcamentoEntity) {
    try {

      return new Promise((resolve, reject) => {
        this._http.post(Constants.API_URL + 'lancarOrcamentoServicoPublicidade/'
          + localStorage.getItem(Constants.TOKEN_USUARIO), JSON.stringify(orcamentoEntity), this.options)
          .map(res=>res.json())
          .subscribe(data => {
            resolve(data);
  ...
  public avaliaCotacao(avaliaCotacaoEntity) {
    try {
        return new Promise((resolve, reject) => {
          this._http.post(Constants.API_URL + 'avaliaCotacao/'
            + localStorage.getItem(Constants.TOKEN_USUARIO), JSON.stringify(avaliaCotacaoEntity), 
            this.options)
            .map(res=>res.json())
            .subscribe(data => {
              resolve(data);
