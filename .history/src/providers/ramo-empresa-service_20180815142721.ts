...
export class RamoEmpresaService {
  ...
  public saveUsuarioRamoSelecionado(usuarioRamoEmpresaEntity) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        + localStorage.getItem(Constants.TOKEN_USUARIO), JSON.stringify(usuarioRamoEmpresaEntity), this.options)
          .subscribe(data => {
            resolve(data);
          }, (err) => {
            reject(err.json());
