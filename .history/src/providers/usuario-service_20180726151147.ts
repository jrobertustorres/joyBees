...
export class UsuarioService {
  ...
  public atualizaUsuario(dadosUsuario) {
      try {
  
        // let salarioFormat = this.dadosUsuarioForm.value.salario.replace(",", "");
        // dadosUsuario.salario = dadosUsuario.salario.replace(",", "");
  
        return new Promise((resolve, reject) => {
          this._http.post(Constants.API_URL + 'editaUsuario/'
          + localStorage.getItem(Constants.TOKEN_USUARIO), JSON.stringify(dadosUsuario), this.options)
            .map(function (res) { return res.json(); })
            .subscribe(data => {
              resolve(data);
              ...
              this.userChangeEvent.emit(dadosUsuario.nomePessoa);
              this.emailPessoaChangeEvent.emit(dadosUsuario.emailUsuario);
            }, (err) => {
              reject(err.json());
            });
        });
