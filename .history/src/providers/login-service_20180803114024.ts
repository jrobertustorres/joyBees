...
export class LoginService {
  ...
  public login(usuarioEntity) {
    try {
      ...
      this.usuarioEntity.idiomaUsuario = localStorage.getItem(Constants.IDIOMA_USUARIO) == 'pt' ? 'PORTUGUES' : 'INGLES';
      ...
      return new Promise((resolve, reject) => {
        this.http.post(Constants.API_URL + 'login/', JSON.stringify(this.usuarioEntity), this.options)
          ...
          .subscribe(data => {
            resolve(data);
            console.log(data.idiomaUsuario);
            data.idiomaUsuario = data.idiomaUsuario == 'Português' ? 'pt' : 'en';
