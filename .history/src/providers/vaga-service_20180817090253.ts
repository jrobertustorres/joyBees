...
export class VagaService {
  ...
  public findVagaDestaqueByVaga(limiteDados) {
    try {

      return new Promise((resolve, reject) => {
        // this.http.post(Constants.API_URL  + 'findVagaByVaga/', this.options)
        this.http.post(Constants.API_URL  + 'findVagaDestaqueByVaga/' + limiteDados, this.options)
          .map(res=>res.json())
          .subscribe(data => {
            resolve(data);
          }, (err) => {
  ...
  public findVagaDetalhe(idVagaUsuario) {
    try {
  ...
  public findVagaCandidatarByVagaUsuario(vagaListaEntity) {
    try {
